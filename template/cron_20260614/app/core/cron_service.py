...
class CronService:
    ...
    def _load_jobs(self) -> tuple[list[CronJob], int] | None:
        """Load jobs from disk. Returns (jobs, version) or None if corrupt."""
        jobs: list[CronJob] = []
        version = 1
        if self.store_path.exists():
            try:
                ...
                logger.exception(
                    "Failed to load cron store at {}. Corrupt file preserved at {}.",
                    self.store_path,
                    backup,
                )
                return None
    ...
    def _merge_action(self):
        """Merge pending actions from action.jsonl."""
    ...
    def _load_store(self) -> CronStore | None:
        """Load jobs from disk. Reloads if file was modified externally."""
    ...
    def _save_store(self) -> None:
        ...
        data = {
            ...
            "jobs": [j.to_dict() for j in self._store.jobs]
        }
    ...
    def add_job(
        ...
        if self._running:
            ...
            self._append_action("add", job.to_dict())
        ...
        for job in store.jobs:
            if job.id == job_id:
                ...
                if self._running:
                    ...
                    self._append_action("update", job.to_dict())
                return job
        return None
